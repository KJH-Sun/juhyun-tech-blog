...
| 관찰 | 근거 |
| --- | --- |
| B가 이긴 4문항은 큰 폭으로 이김 (-14% ~ -64%) | m1, m4, m6, m8: 평균 **-37.2%** 호출 절감 |
| A가 이긴 6문항은 작은 폭으로 이김 (절대값 기준 +1~+6 calls) | 단일 파일에 답이 있으면 Grep 한두 번이면 끝남 |
| 총합이 B 쪽으로 +9.7%인 것은 A의 승수가 많기 때문 | 절대 차이는 작음 (62 vs 68 calls over 10Q) |
...
**핵심 발견**: architecture 카테고리만 카테고리 수준에서도 명확히 B가 우세합니다. 나머지 카테고리는 질문별 편차가 커서 평균으로는 판정 불가합니다.
